...
(function() {
  ...
  const scrollto = (el) => {
    ...
    // getBoundingClientRect() a vizuális pozíciót adja vissza (zoom-ot is figyelembe veszi),
    // míg offsetTop nem — ezért zoom: 0.8 esetén offsetTop hibás eredményt adna.
    // Extra 20px lélegzetvétel a header és a szekció cím között.
    let rect = select(el).getBoundingClientRect()
    window.scrollTo({
      top: window.scrollY + rect.top - offset - 20,
      behavior: 'smooth'
    })
  }
  ...
  window.addEventListener('load', () => {
    if (window.location.hash) {
      if (select(window.location.hash)) {
        // Kis késleltetés kell, hogy a zoom: 0.8 CSS teljesen érvényesüljön
        // a getBoundingClientRect() hívás előtt — különben enyhe pozícióhiba lép fel.
        setTimeout(() => scrollto(window.location.hash), 150)
      }
    }
  });
